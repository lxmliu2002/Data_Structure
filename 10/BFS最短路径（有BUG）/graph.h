...
public:
      ...
      {// 寻找一条从顶点到终点的路径
       // 返回一个数组path，从索引1开始表示路径。path[0]表示路径的长度
       // 如果路径不存在，返回NULL
       // 为寻找路径的递归算法初始化
         int n = numberOfVertices();
         path = new int [n + 1];
         path[1] = theSource;      // 第一个顶点总是theSource
         length = 1;               // 当前路径长度 + 1
      ...
      int* findPathBFS(int theSource, int theDestination)
      {// 寻找一条从顶点到终点的路径
       // 返回一个数组path，从索引1开始表示路径。path[0]表示路径的长度
       // 如果路径不存在，返回NULL
       // 为寻找路径的递归算法初始化
          ...
          // 搜索路径
          if (theSource == theDestination || rFindPathBFS(theSource))
              // 找到一条路径
              path[0] = length - 1;
          else
          {
      ...
      bool rFindPathBFS(int s)
      {// 寻找路径的实际算法，从顶点s开始实施宽度优先搜索
       // 顶点s不应该等于终点
       // 当且仅当一条路径找到了，返回true
          reach[s] = 1;
          arrayQueue<int> q(10);
          ...
          {// 从队列中删除一个标记过的顶点
              int w = q.front();
              q.pop();
              vertexIterator<T>* is = iterator(w);
              int u;
              while ((u = is->next()) != 0)
                  // 访问顶点w的一个相邻顶点
                  if (reach[u] == 0)
                  {// u是一个没有到达过的顶点
                      q.push(u);
                      path[++length] = u; // 路径中加入u
                      if (u == destination || rFindPathBFS(u))
                          return true;
                      // 从顶点u到终点没有路径
                      length--;    // 从路径中删除u
                  }
